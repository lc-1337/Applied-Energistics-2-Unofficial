...
public class PacketVirtualSlot extends AppEngPacket {
    ...
    public PacketVirtualSlot(final Int2ObjectMap<IAEStack<?>> slotStacks) {
    ...
    public PacketVirtualSlot(final int slotIndex, final IAEStack<?> stack) {
        this.slotStacks = null;
    ...
    public void clientPacketData(INetworkInfo network, AppEngPacket packet, EntityPlayer player) {
        if (Minecraft.getMinecraft().currentScreen instanceof IVirtualSlotHolder container) {
            container.receiveSlotStacks(this.slotStacks);
        }
    }
}
